function ri_ci_plot(y0,y1,beta,CI,varargin)
	...
	addOptional(options,'Support',[-inf, inf]); % boundary values for y.
	...
	x1_lb = max( x1 - lb , ones(size(x1))*Support(1)) ;
	x1_ub = min(ones(size(x1))*Support(2) , x1 + ub)  ;
	%  TODO:  improve interpolation at edges.  
	%  Specifically, don't taper to corners, but follow slope of the primary estimates until they hit the Support [0,1].
